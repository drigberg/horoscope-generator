    ...
    this.generateParagraph = function() {
        ...
        for (var n = 0; n < this.structure.length - 1; n ++) {
            ...
            if (currentSentenceType in this.sentenceTypes) {
                ...
                var newSentence = this.cleanSentence(this.generateSentence(this.structure[n]));
                this.paragraph += newSentence;
            }
        }
        return this.paragraph;
    };
    this.generateSentence = function(sentenceType) {
        // could be refactored to act recursively and create a tree rather than an unnested array
    ...
    this.evaluateSign = function(date) {
        ...
        if (moment(date).format() !== 'Invalid date') {
            if (moment(date).format('MMM') in this.calendar) {
                var firstInMonth = this.calendar[moment(date).format('MMM')]['first'];
                var secondInMonth = this.calendar[moment(date).format('MMM')]['second'];
                var split = this.calendar[moment(date).format('MMM')]['divide'];
                var sign = (parseInt(moment(date).format('DD')) <= split ? firstInMonth : secondInMonth);
            }
        }
        return sign;
    };
